...
one of that sector's sub-sectors. Output ONLY valid JSON — no commentary,
no markdown fences.
...
def _validate_pick(
    ...
    if sub not in taxonomy[sec]:
        # Snap to the sector's Other bucket if Gemini hallucinated a sub
...
def classify_batch(
    ...
    if data.get("taxonomy_hash") != tax_h:
        # Taxonomy changed — invalidate the existing cache so picks are
        # re-validated against the new sub-sectors.
